    ...
    async function fetchData() {
      try {
        ...
        if (readingsData.length > 0) {
          const start = format(
            new Date(readingsData[readingsData.length - 1].date),
            "yyyy-MM-dd"
          );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          subtitle={
            previousReading
              ? `${((latestReading.consumption - previousReading.consumption) / (previousReading.consumption || 1) * 100).toFixed(1)}% vs yesterday`
              : ""
          }
        ...
        <StatCard
          ...
          subtitle={bill ? `₱${bill.effectiveRate.toFixed(4)}/kWh effective rate` : ""}
